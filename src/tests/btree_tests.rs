...
fn btree_single_leaf_insert_lookup() {
    let bm = Arc::new(BufferManager::new_with_pages("/tmp/ferric_btree_test".to_string(), 4, 4, 2).unwrap());
...
fn btree_split_and_lookup() {
    let bm = Arc::new(BufferManager::new_with_pages("/tmp/ferric_btree_split".to_string(), 32, 16, 4).unwrap());
    let tree = BTree::new(bm.clone()).unwrap();
