...
fn dirty_batch_write_clears_dirty_and_evicts() {
    let bm =
        BufferManager::new_with_pages("/tmp/ferric_evict_phase".to_string(), 16, 8, 2).unwrap();
    ...
    assert!(
        st == crate::memory::page_state::state::EVICTED
            || st == crate::memory::page_state::state::UNLOCKED
            || st == crate::memory::page_state::state::MARKED
    );
